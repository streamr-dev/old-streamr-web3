...
function deployContracts(code, constructorParams, from) {
    ...
    _(contracts).each(c => {
        ...
        var gas = web3.eth.estimateGas({data: c.bytecode})
        var data = c.bytecode
        // TODO: check and typecast constructorParams (e.g. BigIntegers)
        var p = new Promise((done, fail) => {
            var instance = Contract.new(...constructorParams, {from, data, gas})
